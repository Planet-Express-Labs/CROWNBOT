# This is based upon the original code here: https://github.com/MujyKun/WeverseBot/
...
from CROWNBOT.config import WEVERSE_USERNAME, WEVERSE_PASSWORD, WEVERSE_API_KEY, WEVERSE_IMAGE_STORE
from cogs.translate import translate_text_target
# TODO: refactor
from CROWNBOT.database.weverse_channel import WeverseChannel as TextChannel
...
class Weverse(commands.Cog):
    def __init__(self, bot):
        ...
        client_kwargs = {
            ...
            "authorization": WEVERSE_API_KEY,
            "username": WEVERSE_USERNAME,  # username to log in
            "password": WEVERSE_PASSWORD,  # password to log in
            "loop": loop,  # current event loop
            'hook': self.on_new_notifications
        }

        self._weverse_image_folder = WEVERSE_IMAGE_STORE
        self._upload_from_host = getenv("UPLOAD_FROM_HOST")

        self.weverse_client = WeverseClientAsync(**client_kwargs)
        loop.create_task(self.weverse_client.start())
    ...
    async def set_comment_embed(self, notification, embed_title):
        ...
        if not comment_body:
            ...
            if artist_comments:
                ...
                return
        # TODO: Migrate this to use Naver's API as opposed to Weverse's API. Wevere's translation is a fucking nightmare
    ...
    async def set_post_embed(self, model_object: Union[models.Notification, models.Post, int], embed_title):
        ...
        # TODO: Migrate this to use Naver's translation API instead of using the Weverse API.
